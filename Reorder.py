...
class CSR: 
    ...
    def blocking(self,grouping):
        ...
        for new_idx,old_idx in enumerate(induced_row_order):
            if grouping[old_idx] != current_group:
                ...
                for row_idx in induced_row_order[new_idx - current_size: new_idx]:
                    row = self.pos[row_idx]
                    row_fake_nz = len(np.setdiff1d(current_pattern, row, assume_unique = True))
                    fake_nz += row_fake_nz
            ...
            current_row = self.pos[old_idx]
            current_pattern = np.union1d(current_pattern,current_row);
            current_size += 1
        return fake_nz, current_group +1
    ...
    def blocking_show(self,grouping, sim, filename = "blocking_structure_example.txt"):
        ...
        with open(filename, "w") as outfile:
            ...
            density = graph.tot_nz()/(graph.tot_nz() + fake_nz)
            print(f"******************BLOCKING COMPLETED", file = outfile)
            print(f"BLOCKS: {n_of_blocks} of AVG. SIZE: {graph.N/n_of_blocks}", file = outfile)
            print(f"TRUE NONZEROS: {graph.tot_nz()} FAKE NONZEROS : {fake_nz}, with AVG. in-block DENSITY: {density}", file = outfile)
...
for m in [10,20,50,100]:
    for density in [0.01,0.05,0.1,0.2,0.5]:
        graph = make_random_CSR(n,m,density)
        ...
        for sim_name in similarities: 
            ...
            for tau in np.linspace(0,1,20):
                ...
                if "special" in sim_name:
                    tau *= 3
                do_merge = True
                sim_measure = lambda x,y,z : similarities[sim_name](x,y,z, tau);
